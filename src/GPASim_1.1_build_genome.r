...
##############################################################
nLociPerChrom = round(nLoci / nChrom) #may cause the number of positions sampled not equal to nLoci -> fixed by the if-statement below
counter_chrom=1
for (i in chrom_len_cM){
  if (i != chrom_len_cM[nChrom]){ ### testing if we are at the last chromosome
    out = matrix(sort(sample(seq(from=0.00001, to=i, by=0.00001), size=LocierChrom)), nrow=1)
    ...
    out = matrix(sort(sample(seq(from=0.00001, to=i, by=0.00001), size=(LocierChrom + (nLoci - (nChrom*LocierChrom))))), nrow=1)
  }
