...
### Output:
  ...
  ### AS WELL AS THE WILDTYPE SPEC FILES (P(allele_max)=0.00)
...
qtl_effects = c()
for (i in 1:nQTL){
  ### random sampling of nAlleles^2 and exclude zero effect QTL
  if (allele_eff_model == "CHISQ"){
    eff_temp = round(rchisq(n=nAlleles^2, df=1),2)
  } else if (allele_eff_model == "NORM"){
    eff_temp =  round(rnorm(n=nAlleles^2, mean=0, sd=1),2)
  } else if (allele_eff_model == "UNIF"){
    eff_temp =  round(runif(n=nAlleles^2, min=0, max=5),2)
  } else {
    print(paste0("#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"))
  ...
  ### exclude zero effect QTL
  ### and select nAlleles
  qtl_effects = c(qtl_effects, eff_temp[eff_temp != 0][1:nAlleles])
}
...
###############################################################################
### SET INITIAL ALLELE FREQUENCY OF LARGEST EFFECT ALLELE PER QTLOCUS TO 1% ###
###############################################################################
# Ne = 1000
# freq_min = 1/(2*Ne)
freq_min = 0.01 ### arbitrarily set to 1% implying standing genetic variation
...
for (i in 1:BGS_nQTL){
  ...
  ### exclude zero effect QTL
  ### and select nAlleles
  bgs_effects = c(bgs_effects, eff_temp[eff_temp != 0][1:nAlleles])
}
...
#################################################################################
### SET INITIAL ALLELE FREQUENCY OF LARGEST EFFECT ALLELE PER BGS LOCUS TO 1% ###
#################################################################################
# Ne = 1000
# freq_min = 1/(2*Ne)
freq_min = 0.01 ### arbitrarily set to 1% implying standing genetic variation
...
#######################################################################################################
sub = data.frame(allele_ID=rep(1:nLoci, each=nAlleles)[idxQTL[idxQTL_nonZero_iniFreq]], allele_EFF=col_allelic_value[idxQTL[idxQTL_nonZero_iniFreq]])
MIN_GEBV = 2*sum(aggregate(sub$allele_EFF ~ sub$allele_ID, FUN=min)[,2])
MAX_GEBV = 2*sum(aggregate(sub$allele_EFF ~ sub$allele_ID, FUN=max)[,2])
